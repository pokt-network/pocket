...
func TestHotstuff4Nodes1BlockHappyPath(t *testing.T) {
	...
	// NewRound
	newRoundMessages := WaitForNetworkConsensusMessage(t, testPocketBus, types.P2P_BROADCAST_MESSAGE, consensus.NewRound, numNodes, 1000)
	for _, pocketNode := range pocketNodes {
	...
	// Prepare
	prepareProposal := WaitForNetworkConsensusMessage(t, testPocketBus, types.P2P_BROADCAST_MESSAGE, consensus.Prepare, 1, 1000)
	for _, pocketNode := range pocketNodes {
		nodeState := pocketNode.ConsensusMod.GetNodeState()
	...
	// Precommit
	prepareVotes := WaitForNetworkConsensusMessage(t, testPocketBus, types.P2P_SEND_MESSAGE, consensus.Prepare, numNodes, 1000)
