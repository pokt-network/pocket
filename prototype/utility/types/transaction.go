...
func (x *Transaction) ValidateBasic() Error {
	...
	publicKey, err := crypto2.NewPublicKeyFromBytes(x.Signature.PublicKey)
	if err != nil {
...
func (x *Transaction) Sign(privateKey crypto2.PrivateKey) Error {
...
func TransactionHash(transactionProtoBytes []byte) string {
	return hex.EncodeToString(crypto2.SHA3Hash(transactionProtoBytes))
}
