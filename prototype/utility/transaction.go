...
func (u *UtilityContext) ApplyTransaction(tx *types2.Transaction) types2.Error {
...
func (u *UtilityContext) CheckTransaction(transactionProtoBytes []byte) error {
	...
	if u.Mempool.Contains(txHash) {
		return types2.ErrDuplicateTransaction()
	}
	store := u.Store()
...
func (u *UtilityContext) AnteHandleMessage(tx *types2.Transaction) (msg types2.Message, err types2.Error) {
	msg, err = tx.Message()
	if err != nil {
		return nil, err
	}
	...
	if accountAmount.Sign() == -1 {
		return nil, types2.ErrInsufficientAmountError()
	}
	...
	if !isValidSigner {
		return nil, types2.ErrInvalidSigner()
	}
...
func (u *UtilityContext) HandleMessage(msg types2.Message) types2.Error {
	switch x := msg.(type) {
	...
	default:
		return types2.ErrUnknownMessage(x)
	}
}

func (u *UtilityContext) GetSignerCandidates(msg types2.Message) (candidates [][]byte, err types2.Error) {
	switch x := msg.(type) {
	case *types2.MessageDoubleSign:
