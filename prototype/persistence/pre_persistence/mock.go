...
func (m *MockPersistenceContext) AppHash() ([]byte, error) {
	...
	for ; it.Valid(); it.Next() {
		...
		if len(result) >= 100000 {
			result = crypto2.SHA3Hash(result)
		}
	}
	it.Release()
	return crypto2.SHA3Hash(result), nil
}
