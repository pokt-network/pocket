...
func TestSlip_DeriveChild_TestVectors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Test key derivation is deterministic for path `m` (master key)",
			args: args{
				path: "m",
		...
		{
			name: "Test key derivation is deterministic for path `m/0'`",
			args: args{
				path: "m/0'",
				seed: testVector1SeedHex,
			},
		...
		{
			name: "Test key derivation is deterministic for path `m/0'/1'`",
			args: args{
				path: "m/0'/1'",
				seed: testVector1SeedHex,
			},
		...
		{
			name: "Test key derivation is deterministic for path `m/0'/1'/2'`",
			args: args{
				path: "m/0'/1'/2'",
				seed: testVector1SeedHex,
			},
		...
		{
			name: "Test key derivation is deterministic for path `m/0'/1'/2'/2'`",
			args: args{
				path: "m/0'/1'/2'/2'",
				seed: testVector1SeedHex,
			},
		...
		{
			name: "Test key derivation is deterministic for path `m/0'/1'/2'/2'/1000000000'`",
			args: args{
		...
		{
			name: "Test key derivation fails with invalid path `m/0`",
			args: args{
				path: "m/0",
				seed: testVector1SeedHex,
			},
		...
		{
			name: "Test key derivation is deterministic for path `m/0'/2147483647'`",
			args: args{
				path: "m/0'/2147483647'",
				seed: testVector2SeedHex,
			},
		...
		{
			name: "Test key derivation is deterministic for path `m/0'/2147483647'/1'`",
			args: args{
		...
		{
			name: "Test key derivation is deterministic for path `m/0'/2147483647'/1'/2147483646'`",
			args: args{
		...
		{
			name: "Test key derivation is deterministic for path `m/0'/2147483647'/1'/2147483646'/2'`",
			args: args{
	...
	for _, tv := range tests {
		t.Run(tv.name, func(t *testing.T) {
			seed, err := hex.DecodeString(tv.args.seed)
			require.NoError(t, err)
			childKey, err := crypto.DeriveChild(tv.args.path, seed)
			if tv.wantErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
