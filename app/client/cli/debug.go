...
func NewDebugCommand() *cobra.Command {
	return &cobra.Command{
		...
		PersistentPreRun: func(cmd *cobra.Command, args []string) {
			...
			// TECHDEBT: should we be concerned about this assertion failing?
			// Does it make more sense for `P2PModule#Create` or the static
			// `Create` helper in the same package return a `modules.P2PModule`
			// type rather than a `modules.Module`?
