...
func (m *consensusModule) isQCValid(qc *QuorumCertificate) bool {
	...
	for _, partialSig := range qc.ThresholdSignature {
		validator, ok := valMap[string(partialSig.NodeId)] // TODO(HACK): Need a mapping from address to NodeID or something
		if !ok {
		...
		if !messageToJustify.IsSignatureValid(pubKey, partialSig.Signature) {
