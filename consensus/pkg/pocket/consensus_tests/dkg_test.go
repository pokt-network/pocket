...
func TestDistributedKeyGenerationCrypto(t *testing.T) {
	...
	rnd1Bcast := WaitFoNetworkDKGMessages(t, testPocketBus, events.P2P_BROADCAST_MESSAGE, dkg.DKGRound2, numNodes, 1000)
	rnd1P2p := WaitFoNetworkDKGMessages(t, testPocketBus, events.P2P_SEND_MESSAGE, dkg.DKGRound2, numNodes*(numNodes-1), 1000)
	for _, pocketNode := range pocketNodes {
	...
	rnd2Bcast := WaitFoNetworkDKGMessages(t, testPocketBus, events.P2P_BROADCAST_MESSAGE, dkg.DKGRound3, numNodes, 500)
	for _, message := range rnd2Bcast {
		require.Nil(t, message.Recipient, "DKG broadcast message must not have a recipient.")
		P2PBroadcast(pocketNodes, message)
	}

	rnd3Bcast := WaitFoNetworkDKGMessages(t, testPocketBus, events.P2P_BROADCAST_MESSAGE, dkg.DKGRound4, numNodes, 500)
