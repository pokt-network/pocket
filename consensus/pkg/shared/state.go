...
func (ps *pocketState) LoadStateFromConfig(cfg *config.Config) {
	...
	if cfg.Persistance == nil || len(cfg.Persistance.DataDir) == 0 {
		log.Println("[TODO] Load pocket state from persistance. Only supporting loading pocket state from genesis file for now.")
