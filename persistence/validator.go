...
func (p PostgresContext) GetValidatorExists(address []byte) (exists bool, err error) {
	...
	if err = conn.QueryRow(ctx, schema.ValidatorExistsQuery(hex.EncodeToString(address))).Scan(&exists); err != nil {
		return
	}
	return
}

func (p PostgresContext) GetValidator(address []byte) (operator, publicKey, stakedTokens, serviceURL, outputAddress string, pausedHeight, unstakingHeight, height int64, err error) {
	ctx, conn, err := p.DB.GetCtxAndConnection()
	if err != nil {
		return
	}
