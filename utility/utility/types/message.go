...
func ValidateAddress(address []byte) Error {
	...
	if len(address) != crypto.AddressLen {
		return ErrInvalidAddressLen(crypto.ErrInvalidAddressLen())
	}
	return nil
}
...
func ValidatePublicKey(publicKey []byte) Error {
	...
	if len(publicKey) != crypto.PublicKeyLen {
		return ErrInvalidPublicKeylen(crypto.ErrInvalidPublicKeyLen())
	}
	return nil
}
...
func ValidateHash(hash []byte) Error {
	...
	if len(hash) != crypto.SHA3HashLen {
		return ErrInvalidHashLength(crypto.ErrInvalidHashLen())
	}
	return nil
}
