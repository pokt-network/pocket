...
func (u *UtilityContext) HandleMessageStakeApp(message *types.MessageStakeApp) types.Error {
	...
	if err := u.AddPoolAmount(types.AppStakePoolName, amount); err != nil {
		return err
	}
...
func (u *UtilityContext) HandleMessageEditStakeApp(message *types.MessageEditStakeApp) types.Error {
	...
	// move funds from account to pool
	if err := u.AddPoolAmount(types.AppStakePoolName, amountToAdd); err != nil {
		return err
	}
	// calculate maximum relays from stake amount
...
func (u *UtilityContext) HandleMessageUnstakeApp(message *types.MessageUnstakeApp) types.Error {
	...
	if err := u.SetAppUnstakingHeightAndStatus(message.Address, unstakingHeight, types.UnstakingStatus); err != nil {
		return err
	}
	return nil
}
...
func (u *UtilityContext) UnstakeAppsThatAreReady() types.Error {
	...
	for _, app := range appsReadyToUnstake {
		if err := u.SubPoolAmount(types.AppStakePoolName, app.GetStakeAmount()); err != nil {
			return err
		}
...
func (u *UtilityContext) HandleMessagePauseApp(message *types.MessagePauseApp) types.Error {
...
func (u *UtilityContext) HandleMessageUnpauseApp(message *types.MessageUnpauseApp) types.Error {
	...
	if err := u.SetAppPauseHeight(message.Address, types.ZeroInt); err != nil {
		return err
	}
	return nil
}
...
func (u *UtilityContext) InsertApplication(address, publicKey, output []byte, maxRelays, amount string, chains []string) types.Error {
	store := u.Store()
	err := store.InsertApplication(address, publicKey, output, false, types.StakedStatus, maxRelays, amount, chains, types.ZeroInt, types.ZeroInt)
	if err != nil {
...
func (u *UtilityContext) GetAppsReadyToUnstake() (apps []modules.UnstakingActor, err types.Error) {
	...
	unstakingApps, er := store.GetAppsReadyToUnstake(latestHeight, types.UnstakingStatus)
	if er != nil {
...
func (u *UtilityContext) UnstakeAppsPausedBefore(pausedBeforeHeight int64) types.Error {
	...
	er := store.SetAppsStatusAndUnstakingHeightPausedBefore(pausedBeforeHeight, unstakingHeight, types.UnstakingStatus)
	if er != nil {
...
func (u *UtilityContext) GetMessageStakeAppSignerCandidates(msg *types.MessageStakeApp) (candidates [][]byte, err types.Error) {
...
func (u *UtilityContext) GetMessageEditStakeAppSignerCandidates(msg *types.MessageEditStakeApp) (candidates [][]byte, err types.Error) {
...
func (u *UtilityContext) GetMessageUnstakeAppSignerCandidates(msg *types.MessageUnstakeApp) (candidates [][]byte, err types.Error) {
	output, err := u.GetAppOutputAddress(msg.Address)
	if err != nil {
		return nil, err
	}
...
func (u *UtilityContext) GetMessageUnpauseAppSignerCandidates(msg *types.MessageUnpauseApp) (candidates [][]byte, err types.Error) {
	output, err := u.GetAppOutputAddress(msg.Address)
	if err != nil {
		return nil, err
	}
...
func (u *UtilityContext) GetMessagePauseAppSignerCandidates(msg *types.MessagePauseApp) (candidates [][]byte, err types.Error) {
	output, err := u.GetAppOutputAddress(msg.Address)
	if err != nil {
		return nil, err
	}
