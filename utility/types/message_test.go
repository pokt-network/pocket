...
func TestMessage_ChangeParameter_ValidateBasic(t *testing.T) {
	...
	require.Equal(t, pokterrors.UtilityErrEmptyAddress().Code(), msgMissingOwner.ValidateBasic().Code())

	msgMissingParamKey := proto.Clone(&msg).(*MessageChangeParameter)
	msgMissingParamKey.ParameterKey = ""
	require.Equal(t, pokterrors.UtilityErrEmptyParamKey().Code(), msgMissingParamKey.ValidateBasic().Code())

	msgMissingParamValue := proto.Clone(&msg).(*MessageChangeParameter)
	msgMissingParamValue.ParameterValue = nil
	require.Equal(t, pokterrors.UtilityErrEmptyParamValue().Code(), msgMissingParamValue.ValidateBasic().Code())
}
...
func TestMessage_EditStake_ValidateBasic(t *testing.T) {
	...
	expectedErr := pokterrors.UtilityErrInvalidAddressLen(crypto.ErrInvalidAddressLen(defaultUnusedLength))
	...
	require.Equal(t, pokterrors.UtilityErrEmptyRelayChains().Code(), er.Code())
	...
	expectedErr = pokterrors.UtilityErrInvalidRelayChainLength(0, relayChainLength)
	require.Equal(t, expectedErr.Code(), er.Code())
}
...
func TestMessage_Stake_ValidateBasic(t *testing.T) {
	...
	require.Equal(t, pokterrors.UtilityErrEmptyPublicKey().Code(), er.Code())
	...
	require.Equal(t, pokterrors.UtilityErrNilOutputAddress().Code(), er.Code())
}
